        ...
        <div class="max-w-3xl mx-auto sm:px-6 lg:px-8 space-y-6">
            <div class="bg-white dark:bg-gray-800 overflow-hidden shadow-sm sm:rounded-lg">
                <div class="p-6 border-b border-gray-200 dark:border-gray-700">
                    <h3 class="text-lg font-medium text-gray-900 dark:text-gray-100">{{ __('Ringkasan') }}</h3>
                </div>
                <div class="p-6 overflow-x-auto">
                    <table class="min-w-full divide-y divide-gray-200 dark:divide-gray-700">
                        ...
                        <tfoot>
                            <tr>
                                <td colspan="3" class="px-2 py-3 text-right text-sm font-semibold text-gray-700 dark:text-gray-300">{{ __('Total') }}</td>
                                <td class="px-2 py-3 text-right text-sm font-bold text-indigo-600 dark:text-indigo-400">Rp {{ number_format($cartTotal, 0, ',', '.') }}</td>
                            </tr>
                        </tfoot>
                    </table>
                </div>
                ...
                <div class="p-6">
                    <form method="post" action="{{ route('pos.checkout.store') }}" class="space-y-4">
                        ...
                        <div>
                            ...
                            <p class="mt-1 text-xs text-gray-500 dark:text-gray-400">{{ __('Minimal sama dengan total: Rp') }} {{ number_format($cartTotal, 0, ',', '.') }}</p>
                            <x-input-error class="mt-2" :messages="$errors->get('pay_total')" />
                        </div>
                        <div class="flex items-center gap-2">
                            <x-primary-button>{{ __('Simpan Transaksi') }}</x-primary-button>
                            <a href="{{ route('pos.index') }}" class="text-sm text-gray-600 dark:text-gray-400 hover:underline">{{ __('Kembali ke POS') }}</a>
                        </div>
                    </form>
                </div>
            </div>
